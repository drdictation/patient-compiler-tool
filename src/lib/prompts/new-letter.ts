...
- **STRICT PROHIBITION ON META-COMMENTARY:** NEVER state or write in the letter that information was omitted, missing, not mentioned, unstated, or not found in the transcript (e.g. NEVER write "not found in transcript", "was not discussed", "no history provided", "not recorded", etc.). Simply omit the detail silently.
...
- Do not add disclaimers, commentary, meta-commentary, or explanations outside or inside the letter.
...
**Diet**
[Include diet history very concisely and only where relevant to symptoms or management. Do not present reported food triggers as established causes unless the doctor endorsed this. Really only mention if the patient 1) is excluding food and what food typoes 2) Triggers 3) diet is high or low fibre 4) excessive, normal or low in caffine.]
